...
var isCloudFlareIP = (ip) => {
  ...
  const isIp4InCidr = (ip2, cidr) => {
    const cidrRangeMask = cidrToRangeMask(cidr);
    const [ipRange, ipMask] = cidrRangeMask;
    const ipInt = ip2.split('.').reduce((acc, val) => (acc << 8) + parseInt(val, 10), 0);
    return (ipInt & ipMask) === ipRange;
  };

  return cidrList.some((cidr) => isIp4InCidr(ip, cidr));
};
